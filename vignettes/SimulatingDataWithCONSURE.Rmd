...
* we decide if we want to use robust estimators to estimate survival and migratory connectivity. The default is to use a robust estimator. If we don't want to use that we set robust = FALSE. Robust estimation is robust against outliers and may give better results when there is not much data. The ordinary estimator is a bit more smooth and may perform better when there is less data where some data point look like outliers but are none. It might be tricky to decide which estimator performs better.
