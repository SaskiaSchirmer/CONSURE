...
```{r fig.fullwidth=TRUE}
mP <- plotM(mro,"b1", trueValuesAvailable = TRUE)
...
mro2 <- markRecaptureObject(xrange = c(0,1),
                            yrange = c(0,1),
                             survival = function(w){stmp <-function(w){-sum(0.2*w)+0.9}
                    ...
                    mtmp(b,w = c(w[1]/100222.8,w[2]/123642.5),B=B)},
                            observationTime = 10)
...
plotM(mro2,"b1", trueValuesAvailable = TRUE)
```
