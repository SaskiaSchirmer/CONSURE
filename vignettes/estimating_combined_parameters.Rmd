...
output:
  rmarkdown::html_vignette:
    ...
    toc_depth: 4
vignette: >
  %\VignetteIndexEntry{3EstimatingParametersUsingTheCombinedApproach}
...
plot_comb_m(mro, o_o, zlim = c(0, 2.6), est_corrected = TRUE)
```

## Two-dimensional space

```{r 2d, warning = FALSE, cache = TRUE, eval = FALSE}
B <- 3
...
```{r split2d, eval = FALSE}
split <- c(rep(rep(1:2, each = 50), 50), rep(rep(3:4, each = 50), 50))
```

```{r plotmsr2d, fig.fullwidth=TRUE, fig.width=7, fig.height=5, eval = FALSE}
plot_msr(mro2)
```

```{r knotsprop2d, eval =FALSE}
knots_prop <- list(
