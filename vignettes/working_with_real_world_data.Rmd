...
```
We rename it using the function rename_data and overwrite the data.frame of dead recoveries by the renamed data.frame
...
```
The method estimates a robust linear model to estimate the parameters. If an ordinary linear model should be estimated instead, the argument robust must be set to FALSE when initializing the mark_recapture_object.

## Looking at the raw data

We can look at the pattern of the raw recoveries using the function plot_raw_recoveries and check if e.g., the window matches with the pattern:
...
```{r estimate_survival, warning = FALSE, cache=TRUE}
...
```{r estimate_migratory_connectivity, cache=TRUE}
...
```{r estimate_migratory_connectivity_each_area, warning =FALSE, cache =TRUE}
rwd <- est_m(rwd)
```
...
From this list, we select a subset of data sets, e.g., the data sets 1 - 50, for which we estimate the parameter values using the CONSURE-function est_uncertainty:
```{r uncertainty, warnings= FALSE, cache=TRUE}
start <- 1
end <- 50
...
```{r uncertainty_second_part, cache = TRUE}
start <- 50
end <- 100
