...
plotKDE <- function(b,markRecaptureObject, pdf = FALSE, ylim = c(0,1.5),
  ...
  if(pdf) pdf("KDE.pdf")
    if(dim == 1){
      ...
      if(trueValuesAvailable){
        ...
        tmp2$y <- apply(tmp2,1,
                        function(x) f_f(x["x"],x["age"],b, markRecaptureObject,p))
      ...
      if(b == "all"){print("Not possible to plot true state for b = 'all'")
        ...
        #  kdeGridTrue <- kdeGridTrue[-1,]
          ...
          if(trueValuesAvailable){
            kdeGridTrue <- numeric(4)
            gridTmp <- expand.grid(longitude = seq(xlim[1],xlim[2],
                                                   length.out = res),
                             latitude = seq(ylim[1],ylim[2],length.out = res))
            for(t in 1:T){
              tmp <- gridTmp
              tmp$kde <- apply(gridTmp,1,function(x){f_f(x,t = t, b = b,
                                                         markRecaptureObject,
                                                         p = p)})
              tmp$time <- t
              kdeGridTrue <- rbind(kdeGridTrue,tmp)
          ...
          if(!facetByTime){
            tmp <- tidyr::spread(kdeGridTrue, .data$time, .data$kde)
            kdeGridTrue <- tmp[,1:2]
      ...
      #  ggplot2::geom_tile(data = kdeGrid, ggplot2::aes(longitude, latitude,fill = kde)) +
        ...
        ggplot2::scale_fill_viridis_c("kde",
                                      ...
                                      breaks = my_breaks,
                                      labels = formatC(my_breaks,format="e",digits=1))+
        ggplot2::theme(text = ggplot2::element_text(size = 23))

      if(!trueValuesAvailable){
        pg <- pg + ggplot2::geom_tile(data = kdeGrid,
  ...
  if(pdf){if(pdf) plot(pg); grDevices::dev.off()}
  pg
}
