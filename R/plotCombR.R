#' Plot function for true and estimated continuous, discrete and combined recovery probability
#'
...
#' }
plotCombR <- function(optimizationObject,
                      pdf = FALSE,
    ...
    if(dim ==1){
    if(trueContinuous)   pl <- pl +  ggplot2::stat_function(data = data.frame(x = seq(0,1,le=100)),
                               ggplot2::aes(color = "true"),
                               fun=function(x) rFunc(x))

   # if(trueDiscrete) pl <- pl + ggplot2::geom_line(data.frame(x = seq(0,1,le=100),
   #                             y = rep(prop/0.25, each = 25)),
   #                             mapping = ggplot2::aes(x = x, y= y, color = "discrete true"))

    #if(estDiscrete) pl <- pl + ggplot2::geom_line(data.frame(x = seq(0,1,le=100),
  ...
  ggplot2::scale_color_manual("",breaks = c("true",#"discrete true",
                                        #"discrete estimate",
        ...
        colnames(rGrid) <- c("longitude","latitude","r","breedingArea","dataType")
        if(trueContinuous){
        rGridTrue <- expand.grid(longitude = seq(xlim[1],xlim[2],length.out = res),
                         latitude = seq(ylim[1],ylim[2],length.out = res),
                         breedingArea = b)
        rGridTrue$r <- apply(rGridTrue,1,function(x){rFunc(as.numeric(x[1:2]))})
        ...
        rGrid$dataType <- factor(rGrid$dataType, levels = c("true", "continuous", "combined"))
          ...
          rGrid$dataType <- factor(rGrid$dataType, levels = c("continuous", "combined"))
        }
