#' Plot function for true and estimated continuous, discrete and combined migratory connectivity
#'
#' This function plots (in 1D) combinations of true and estimated migratory connectivity values. It is possible to chose between continuous, discrete and combined estimates.
#'
#' @param markRecaptureObject object of class markRecaptureObject
#' @param optimizationObject object of class optimizationObject
#' @param pdf logical, plot as pdf?, defaults to FALSE
#' @param fileName character string, only if pdf is TRUE, defaults to current time
...
plotCombM <- function(markRecaptureObject,
    ...
    if(trueContinuous) mFunc <- markRecaptureObject$breedingAreas[[b]]$migratoryConnectivity
    ...
    if(dim == 1){
    if(trueContinuous)   pl <- pl +  ggplot2::stat_function(data = data.frame(x = yHelp),
  ...
  ggplot2::scale_color_manual("",breaks = c("true","discrete true","discrete estimate","continuous estimate","corrected estimate","combined estimate"),
    ...
    if(!is.null(zlim)) pl <- pl + ggplot2::lims(y = zlim)
        ...
        colnames(mGrid) <- c("longitude","latitude","m","breedingArea","dataType")
        if(trueContinuous){
        mGridTrue <- expand.grid(longitude = seq(xlim[1],xlim[2],length.out = res),
                         latitude = seq(ylim[1],ylim[2],length.out = res),
                         breedingArea = b)
        mGridTrue$m <- apply(mGridTrue,1,function(x){mFunc(as.numeric(x[1:2]))})
       ...
       # my_breaks <- c(min(mGrid$m),my_breaks, max(mGrid$m))
        ...
        mGrid$dataType <- factor(mGrid$dataType, levels = c("true", "continuous", "combined"))
        ...
        pl <- pl +
             ...
             ggplot2::scale_fill_distiller("connectivity", palette = "Spectral",
                                ...
                                labels = formatC(seq(max(0,my_breaks[1]),my_breaks[11],length.out = 5),format="e",digits=1))+
