...
#' @export
#' @examples mro <- bootstrap_quantiles(mro1D, "s")
#'
bootstrap_quantiles <- function(mark_recapture_object, param = c("s", "m", "r"),
                                filename) {
  ...
  mark_recapture_object$estimates$bootstrap$bootstrap_quantiles <-
    ...
    dplyr::summarise(
      uq = quantile(.data$value, 0.975, na.rm = TRUE),
      lq = quantile(.data$value, 0.025, na.rm = TRUE)
    )
