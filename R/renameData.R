...
#' @param recoveryData data.frame with dead recoveries, containing information on breeding
#'  origin, geographical coordintes of recovery and age at death
...
#' defaults to NULL, do not specify in one-dimensional case, but use xname instead
#' @param timename character, name of column containing information on age at death
...
renameData <- function(recoveryData, xname, yname = NULL, timename, markAreaName){
  ...
  if(is.null(yname)){
    colnamesMatrix <- matrix(c(markAreaName,xname,timename,
                       "markArea", "longitude","age"),
                     ncol = 2, nrow = 3)
  } else{
    colnamesMatrix <- matrix(c(markAreaName,xname,yname,timename,
