...
#' @param f_f_sub function: density of found individuals only
...
simContin <- function(B,
  ...
  for(b in 1:B){
    ...
    # using rejection sampling
    f_f_sub2 <- function(x){
      f_f_sub(w = x[1], t= x[2], b=b, lb = 0, ub = 1, s,f,r,T )
    }
    ...
    eta[[b]] <-rejectionSampling(k[b]+1, df = f_f_sub2, dg = dg, rg = rg, M=10)
